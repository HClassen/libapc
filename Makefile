...
PT= -pthread
DEBUG= -DDEBUG
FILES= src/apc.c src/core.c src/core-linux.c src/net.c src/fd.c src/fs.c src/timer.c src/tcp.c src/udp.c src/threadpool.c src/heap.c 

main: $(FILES)
	$(CC) $(CFLAGS) -o apc main.c $(PT) $(FILES) $(POSIX)

debug: $(FILES)
	$(CC) $(CFLAGS) -o apc main.c $(PT) $(FILES) $(POSIX) $(DEBUG) -g
